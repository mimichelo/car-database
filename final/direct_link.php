...
<head>
    ...
    <title>Displaying Cars by Categories</title>
    <style type="text/css">
        .auto-style1 {
            border-style: none;
...
if ($total != 0) {
    ...
    foreach ($header as $category) {
        echo "<a href='?category=" . strtolower($category) . "'>" . $category . "</a> ";
    }
    echo "</h3>";
    ...
    while ($result = mysqli_fetch_assoc($data)) {
        if ($count % 3 == 0) {
            $category = strtolower($header[$headerIndex]);
        ...
        $count++;
        if (($count % 3 == 0 || $count == $total) && isset($_GET['category']) && $_GET['category'] == $category) {
            echo "</table>";
        }
    }
